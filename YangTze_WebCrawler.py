...
def show_data(data,article,name):
    ...
    for i in list(data.values()):
       plt.text(j,int(i)+k,i,horizontalalignment='center',fontsize=text_size)
       j+=float((end-start)/(len(list(data.values()))-1))
    plt.savefig("%s_%s%s.png" %(time.strftime("%m_%d"),name,classification),dpi=500,format="png")
    plt.close("all")
...
for k in range(1,4):
    ...
    while True:
        ...
        if data1 == []: break    #爬到最後一頁
        data2=html_bp.find_all( text=re.compile('單位 :') ) #擷取單位文字
        data3=html_bp.find_all( text=re.compile('點閱率') )  #擷取點閱率文字
        for data in range(len(data1)):
            #print(data1[data].find('a').text)
            #print(data2[data])
            #print(data3[data])
            #ArticleTitle.append(data1[data].find('a').text)  #目前標題非擷取重點
            Department=re.compile(r' 單位 : (.*)').search(data2[data]).group(1)
